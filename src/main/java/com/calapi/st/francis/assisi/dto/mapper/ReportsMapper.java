...
public class ReportsMapper {
	...
	public Map<String, Object> baptismReportsMapper(BaptismRecord record) {
		...
		params.put("NAME_OF_FATHER", toUpper(joinNames(record.getFatherFirstName(),
				record.getFatherMiddleName(), record.getFatherLastName())));

		params.put("NAME_OF_MOTHER", toUpper(joinNames(record.getMotherFirstName(),
				record.getMotherMiddleName(), record.getMotherLastName())));
	...
	public Map<String, Object> weddingReportMapper(WeddingRecord record) {
		Map<String, Object> params = new HashMap<>();
		params.put("GROOM_P", toUpper(joinNames(record.getGroomFirstName(), record.getGroomMiddleName(),
				record.getGroomLastName())));
		params.put("BRIDE_P", toUpper(joinNames(record.getBrideFirstName(), record.getBrideMiddleName(),
				record.getBrideLastName())));
